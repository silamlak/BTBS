...
import { getScheduleFun } from "../../features/schedule/scheduleApi";
...
const Schedules = () => {
  ...
  const orderedData = useSelector((state) => state.schedule.orderData);
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["schedules", currentPage, limit, searchQuery, dateValue],
    queryFn: () =>
      getScheduleFun({
        limit,
        currentPage,
...
//   console.log(data);
  useEffect(() => {
    if (data) {
      dispatch(addSchedule(data.schedules));
    }
  }, [data]);
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      console.log(data);
      dispatch(deleteScheduleFiles(toDelete));
      setToDelete([]);
