...
import { getDriversFun } from "../../features/drivers/driversApi";
import {
  addDriver,
  deleteDriverFiles,
...
const Drivers = () => {
      ...
      const orderedData = useSelector((state) => state.driver.orderData);
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["drivers", currentPage, limit, searchQuery, dateValue],
    queryFn: () =>
      getDriversFun({
        limit,
        currentPage,
  ...
  useEffect(() => {
    if (data) {
      dispatch(addDriver(data.drivers));
    }
  }, [data]);
  console.log(data);
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      console.log(data);
      dispatch(deleteDriverFiles(toDelete));
      setToDelete([]);
